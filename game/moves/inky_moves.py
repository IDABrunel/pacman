...
class InkyMoves:
    ...
    def get_target_location(self, agent, grid, pacman_location):
        dist = math.sqrt(pow(agent._game.blinky._current_location[0] - pacman_location[0], 2) + pow(agent._game.blinky._current_location[1] - pacman_location[1], 2))
        direction_to_move = {
            "": (0, 0),
            "U": (0, round(2 + dist)),
            "D": (0, -round(2 + dist)),
